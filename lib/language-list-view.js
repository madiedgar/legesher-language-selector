...
class LanguageListView {
    ...
    this.selectListView = new SelectListView({
      ...
      didConfirmSelection: (language) => {
        ...
        if (language === this.autoDetect) {
          ...
          atom.textEditors.setLanguageOverride(this.editor, language.scopeName)
        }
      },
      didCancelSelection: () => {
        this.cancel()
  ...
  async toggle () {
    if (this.panel != null) {
      ...
      this.currentLanguage = this.editor.getLanguage()
      if (this.currentLanguage === atom.languages.nullLanguage) {
        this.currentLanguage = this.autoDetect
      }

      const languages = atom.languages.getLanguages().filter((language) => {
        return language !== atom.languages.nullLanguage && language.name
      })
      languages.sort((a, b) => {
